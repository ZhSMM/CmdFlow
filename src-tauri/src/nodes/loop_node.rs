...
fn default_iter() -> String { "item".to_string() }
...
impl Node for LoopNode {
    ...
    fn display_name(&self) -> &'static str { "循环" }
    fn category(&self) -> &'static str { "control" }
    fn description(&self) -> &'static str { "对集合迭代 (Phase 2 简化版:仅收集 items)" }
    ...
    async fn execute(&self, _ctx: NodeContext, config: Value) -> crate::error::AppResult<NodeOutput> {
        ...
        crate::nodes::node::stream_event(&_ctx.app, &_ctx.execution_id, &_ctx.node_id,
